...
if(isset($_SESSION["username"])){
    echo '<body>
            ...
            <div id="content">
            <div id="container">
                <div id="center">You are already logged in! Taking you to the home page now!</div>
                </div>
            </div>
        </body>';
        ...
        if($stmt = $mysqli->prepare("SELECT username FROM member WHERE username = ?")){
            ...
            if($stmt->fetch()){
                ...
                if($stmt = $mysqli->prepare("INSERT INTO member (username,password,firstname,lastname,email,zipcode) VALUES (?,?,?,?,?,?)")){
                    $stmt->bind_param("sssssi", $_POST["username"], md5($_POST["password"]), $_POST["firstname"], $_POST["lastname"], $_POST["zipcode"]);
                    $stmt->execute();
                    $stmt->close();
